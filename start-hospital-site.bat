...
if not exist "%JAVA_HOME%\bin\java.exe" (
    ...
    echo Please update JAVA_HOME inside start-hospital-site.bat
    pause
    exit /b 1
)
...
echo Starting Banerjee Diagnostic Foundation ^& Hospaccx website...
echo Open http://localhost:8081 after startup completes.
echo Appointment requests will open directly in WhatsApp.
echo.
